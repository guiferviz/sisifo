...
def test_plugins_module_has_correct_members():
    members = [i for i in dir(sisifo.plugins) if i[0] != "_"]
    assert sorted(members) == ["common", "load_plugins"]
...
def test_do_not_load_plugins_on_start_up(mocker):
    ...
    find_plugins_mock = mocker.patch("sisifo.plugins.load_plugins")
    sisifo.plugins._load_plugins_start_up()
